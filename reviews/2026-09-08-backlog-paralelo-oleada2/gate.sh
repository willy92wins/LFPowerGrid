...
echo "-- CRLF --"
# Solo ficheros que siguen existiendo: un borrado no tiene finales de linea que comprobar,
# y `file` sobre una ruta que ya no esta da un FAIL que no significa nada.
for f in $(git diff --name-only --diff-filter=d); do
  if file "$f" | grep -q CRLF; then echo "  ok   $f"; else echo "  FAIL LF $f"; FAIL=1; fi
done
echo "-- borrados (no se comprueba CRLF) --"
git diff --name-only --diff-filter=D | sed 's/^/  D /'
