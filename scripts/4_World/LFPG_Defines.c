...
// ---- v2.0: Battery (energy storage) ----
// Timer runs in NetworkManager (Solar timer pattern). Energy accounting
// uses actual elapsed time, not constant interval, to prevent drift.
...
// Battery tier defaults (overridable per-device via LFPG_Get* interface).
