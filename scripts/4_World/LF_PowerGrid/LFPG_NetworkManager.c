// =========================================================
// LF_PowerGrid - NetworkManager (v0.7.21)
//
...
class LFPG_NetworkManager
{
    ...
    protected void TickPropagation()
    {
        ...
        // H3: Use higher budget during warmup to reduce convergence latency
        int budget = LFPG_PROPAGATE_NODE_BUDGET;
        if (m_WarmupActive)
        {
            budget = LFPG_PROPAGATE_WARMUP_BUDGET;
        }

        int remaining = m_Graph.ProcessDirtyQueue(budget);

        // Clear warmup mode once the queue is fully drained
