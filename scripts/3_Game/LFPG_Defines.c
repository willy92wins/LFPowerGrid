...
static const float LFPG_OCC_RAYCAST_TICK_MS   = 75.0;  // day-10 C5; hysteresis / forced-recheck paths unchanged
...
static const string LFPG_VERSION_STR = "1.2.5";
...
// Kind 1 (final) skips this bucket for one changing final after each
// accepted ordinary AIM for that camera. Extra finals share the 50 ms
// bucket and can be dropped. A final that does not change stored yaw/pitch
// does not spend that token. Server-initiated teardown (power, timeout,
// death) and disconnect ForceCleanup drop a still-pending ordinary.
